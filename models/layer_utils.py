...
class st_encoder(nn.Module):
    def __init__(self, type=False):
        ...
        # self.spatial_conv2 = nn.Conv2d(1, 16, dim_kernel, stride=1, padding=1)
    ...
    def forward(self, X):
        ...
        return: b, F
        '''
        X_t = torch.transpose(X, 1, 2)  #[513, 2, 8]
        X_after_spatial = self.relu(self.spatial_conv(X_t))   #1d卷积 [513, 16, 8]
        X_embed = torch.transpose(X_after_spatial, 1, 2)   #[513, 8, 16]

        output_x, state_x = self.temporal_encoder(X_embed)     #使用GRU网络训练  [513, 8, 64], [513, 64]

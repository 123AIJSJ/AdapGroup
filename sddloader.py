	...
	def __init__(self, set_name="train", b_size=4096, t_tresh=60, d_tresh=50, scene=None, id=False, verbose=False):
		...
		if id==False:
			for t in traj:
				...
				# 	traj_new = t
				# else:
				# 	traj_new = np.concatenate((traj_new, t), axis=0)
				traj_new.append(t)
				if set_name=="train":
					#augment training set with reversed tracklets...
					reverse_t = np.flip(t, axis=1).copy()
					# traj_new = np.concatenate((traj_new, reverse_t), axis=0)
					traj_new.append(reverse_t)
		else:
			for t in traj:
				t = np.array(t)
				# if traj_new.shape(0) < 5:
				# 	traj_new = t
				# else:
				# 	traj_new = np.concatenate((traj_new, t), axis=0)
				traj_new.append(t)
		...
		for m in masks:
			...
			# 	masks_new = m
			# else:
			# 	masks_new = np.concatenate((masks_new, m), axis=0)

...
def print_time(func):
    def new_func(self,*args,**kwargs):
        ...
        end_time = time.time()
        print(self.__class__.__name__,func.__name__, 'lasts',end_time-start_time,'s')
        return res
    return new_func
...
class PastEncoder(nn.Module):
    ...
    def get_group_complete_data(self,weight,ftraj,num_ped):
        '''
        ...
        for i, subgraph in enumerate(complete_subgraphs):
            ...
            feat_c[:, list(num_diff)] = 0
            adj_list.append(feat_c)
            scale_list.append(len(subgraph))
            mask = torch.zeros(len(num_ped), dtype=torch.bool, device=ftraj.device)
            mask[list(num_diff)] = True
            mask_list.append(mask)
        adj_list = torch.stack(adj_list, dim=0)  # graph_num, 11,11
        mask_list = torch.stack(mask_list, dim=0)  # graph_num,11
        subgraph_num = len(complete_subgraphs)
        return subgraph_num, ftraj.expand(subgraph_num,*ftraj.shape[1:]),\
               adj_list, mask_list,torch.tensor(scale_list).type_as(ftraj)
    ...
    def divide_group_adap(self, feat_corr, batch_size, agent_num, ftraj_input, ftraj_inter):
        ...
        for i in range(batch_size):
            feat_co = feat_corr[i]
            sub_ftraj = torch.unsqueeze(ftraj_input[i], dim=0)
            subgraph_num, ftraj, adj_list, mask_list,scale_list = self.get_group_complete_data(feat_co, sub_ftraj, num_ped)
            bs_ftraj.append(ftraj)
            bs_adj.append(adj_list)
            bs_mask.append(mask_list)
            subgraph_num_list.append(subgraph_num)
            bs_scale.append(scale_list)
        bs_ftraj = torch.cat(bs_ftraj,dim=0)
        bs_mask = torch.cat(bs_mask, dim=0)
        bs_adj = torch.cat(bs_adj, dim=0)
        bs_scale = torch.cat(bs_scale,dim=0)
        # self.args.hyper_scales = bs_scale
        # self.args.hyper_scales = [11]
        ftraj_inter_hyper, _ = self.interaction_hyper_angle(bs_ftraj, bs_adj)
        ftraj_inter_hyper[bs_mask] = bs_ftraj[bs_mask]


        # pooling
        feat_bs = []
        start,end = 0,0
        for subgraph_num in subgraph_num_list:
            start,end = end,end+subgraph_num
            # max pooling
            feat_one = ftraj_inter_hyper[start:end].max(dim=0,keepdim=True)[0] # 1,11,d
            feat_bs.append(feat_one)
        feat_bs = torch.cat(feat_bs,dim=0) # bs,11,d
        ####resnet
        output_feature = torch.cat((ftraj_input, ftraj_inter, feat_bs), dim=-1)
    ...
    def forward(self, inputs,batch_size, agent_num):
        ...
        for i in range(0, batch_size):
            a = feat_corr[i].min()
            feat_corr[i] = torch.where(feat_corr[i] < (a + 0.01), ZEROS, feat_corr[i])
        ######
...
class GroupNet(nn.Module):
    ...
    def forward(self,data):
        ...
        pred_loss = loss_pred + loss_recover ##zyz

        return total_loss, loss_pred.item(), loss_recover.item(), loss_kl.item(), loss_diverse.item(), pred_loss
...
def print_time(func):
    def new_func(self,*args,**kwargs):
        ...
        end_time = time.time()
        print(self.__name__,'lasts',end_time-start_time,'s')
    return new_func

...
class MS_HGNN_hyper_angle(nn.Module):
    """Pooling module as proposed in our paper"""
    ...
    def __init__(
        ...
        super(MS_HGNN_hyper_angle, self).__init__()
        ...
        self.nmp_mlp_start = MLP_dict_softmax(input_dim=hdim_extend, output_dim=h_dim, hidden_size=(128,),
        ...
        for i in range(nmp_layers):
            edge_aggregation_list.append(
                edge_aggregation(input_dim=h_dim, output_dim=bottleneck_dim, hidden_size=(128,),
                                 edge_types=self.edge_types))
        self.edge_aggregation_list = nn.ModuleList(edge_aggregation_list)
    ...
    def make_nmp_mlp(self):
        ...
        for i in range(self.nmp_layers - 1):
            mlp1 = MLP(input_dim=self.h_dim * 2, output_dim=self.h_dim, hidden_size=(128,))
            mlp2 = MLP_dict_softmax(input_dim=self.hdim_extend, output_dim=self.h_dim, hidden_size=(128,),
                                    edge_types=self.edge_types)
            nmp_mlp.append(mlp1)
            nmp_mlp.append(mlp2)
        nmp_mlp = nn.ModuleList(nmp_mlp)
    ...
    def init_adj_attention_listall0(self, feat, feat_corr, scale_factor=2):
        batch = feat.shape[0]
        actor_number = feat.shape[1]
        if scale_factor == actor_number:
            H_matrix = torch.ones(batch, 1, actor_number).type_as(feat)
